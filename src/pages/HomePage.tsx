...
const faqTopics: FaqTopic[] = [
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          'Теперь в редакторе доступны два способа работы: визуальная сборка формулы и ручной режим кодом. Это удобно, когда часть пользователей хочет собирать расчёт кнопками, а часть предпочитает писать выражение самостоятельно.',
      ...
      },
      {
        ...
        items: [
          'В верхней панели редактора есть кнопка «Протестировать». Она нужна для быстрой проверки сценария прямо внутри конструктора без лишних переходов.',
          'В тестовом режиме удобно сразу видеть текущие значения полей и итог расчёта, чтобы быстро проверять формулы, пороги, коэффициенты и условия отображения.',
          'Используйте этот режим после каждого заметного изменения формулы, особенно если у вас есть ползунки, чекбоксы, условия и несколько промежуточных результатов.',
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        items: [
          ...
          'Условия и функции: Если, Мин, Макс, Округл и Модуль. Они помогают собирать не только прямую цену, но и правила вроде порогов, ограничений и фиксированных значений.',
        ],
      },
      {
        title: 'Визуальный редактор формул',
        items: [
          'Визуальный режим подходит тем, кто не хочет писать выражение вручную. Формула собирается из полей, переменных, чисел, операторов и функций по шагам.',
          'В левой части доступны поля и переменные вроде «Базовая цена», «Общий коэффициент» и все блоки, участвующие в расчёте. Ниже идут операторы, сравнения и функции.',
          'Справа показывается порядок элементов, итоговое выражение и предварительный результат. Это помогает сразу увидеть, что именно будет считать калькулятор.',
          'Если нужен условный сценарий, используйте функцию Если(условие, значение_если_да, значение_если_нет). Для простых случаев визуальный редактор также корректно преобразует запись вроде «Ползунок < 10 == 3000» в условную формулу.',
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          'Пример 7. Пороговая цена через условие: Если(Ползунок < 10, 3000, Ползунок * 450). Такой сценарий полезен, когда до определённого значения действует фиксированная ставка.',
          'Пример 8. Ограничение итоговой суммы: Макс(Если(Площадь < 1, 1, Площадь) * Базовая цена, 5000). Так можно одновременно задать минимальную площадь и нижний порог по сумме.',
        ],
      },
      {
        title: 'Частые ошибки',
        items: [
          ...
          'Для условных сценариев нельзя использовать запись как в обычной фразе. Вместо «Ползунок меньше 10 то 3000» собирайте условие через Если(...) или через визуальный конструктор.',
          'Если функция возвращает 0 или не срабатывает, проверьте число аргументов. Например, Если требует 3 аргумента, Округл принимает 1 или 2, а Модуль только 1.',
        ],
      },
      {
        ...
        items: [
          ...
          'После сборки обязательно откройте «Протестировать» и пройдите несколько сценариев с разными значениями: минимум, максимум, пороговое значение и заведомо крайние случаи.',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    sections: [
      ...
      {
        title: 'Рассылка обновлений',
        items: [
          'В блоке супер-админки рассылку можно отправлять либо всем подходящим подписчикам сразу, либо только выбранным получателям. Это удобно для тестов, адресных уведомлений и поэтапных запусков.',
          'Получатель может отписаться от рассылки отдельной кнопкой прямо в сообщении. После отписки он не должен попадать в следующий массовый список, пока снова не подпишется.',
          'После повторной подписки кнопка меняется обратно на вариант отписки, чтобы состояние рассылки было понятно прямо в диалоге без лишних уточнений.',
        ],
      },
    ],
  },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          'В новых экранах и вложенных разделах используйте кнопку «Назад», чтобы возвращаться по реальной ветке сценария: например, из шаблонов обратно в «Мои калькуляторы», а не на другой экран.',
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        items: [
          ...
          'Во вложенных экранах шаблонов теперь можно возвращаться кнопкой «Назад» по текущему пути пользователя. Это ускоряет навигацию и уменьшает количество лишних переходов.',
        ],
      },
      {
        title: 'Аналитика',
        items: [
          ...
          'Карточки аналитики адаптированы под десктоп и мобильные экраны: текст и содержимое не должны налезать друг на друга даже при длинных подписях.',
        ],
      },
    ],
  },
